...
function login(mode = "signin", prefill = "") {
  ...
  modal(
    ...
    /* HTML */ `
      ...
      <p class="hint">
        ${creating ? "Create your Berylite account to leave messages for the class." : "Use the username and password for your existing account."}
      </p>
      <form id="login-form">
...
function renderHero() {
  ...
  $("#class-back").innerHTML = media(state.hero[1], "Another side of us");
...
function studentCard(s) {
  ...
  return /* HTML */ `
    ...
    >
      ...
      <h4>${esc(s.name)}</h4>
      <p>${esc(s.quote || "A story waiting to be told.")}</p>
    </button>
  `;
}
